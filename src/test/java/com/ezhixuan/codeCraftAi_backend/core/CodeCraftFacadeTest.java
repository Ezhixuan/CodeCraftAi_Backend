...
class CodeCraftFacadeTest {
    ...
    void chatAndSaveStream() {
        Flux<String> stringFlux = codeCraftFacade.chatAndSaveStream("帮我生成一个任务清单页面,要求能够输入内容回车直接记录下任务,并且提供有序的任务列表,可以通过按钮表示任务完成,完成后进入完成列表并记录完成时间", CodeGenTypeEnum.HTML_MULTI_FILE, 2L);
